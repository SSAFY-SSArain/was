...
public interface BrainMemberRepository extends JpaRepository<BrainMember, BrainMember.BrainMemberId> {
    ...
    Page<BrainMember> findByBmid_BidIn(List<Integer> bid, Pageable pageable);
    ...
    @Query("""
            SELECT 0 < count(*)
            FROM BrainMember bm
            WHERE bm.bmid.bid = (SELECT bid FROM BrainTopic WHERE btid = :btid)
                AND bm.bmid.uid = :uid AND bm.role IN (:role)
            """)
    boolean existsByUidAndBtidAndRoleIn(UUID uid, int btid, List<BrainMemberRole> role);
